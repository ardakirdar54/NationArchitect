...
import com.nationarchitect.game.modules.Building;
import com.nationarchitect.game.modules.BuildingRegistry;
import com.nationarchitect.game.modules.Region;
import com.nationarchitect.game.screens.BaseScreen;

public class EducationPanel extends UIPanel {

    private Region currentRegion;
    private Label regionNameLabel;
    private Array<Building> buildings;
    private int currentPage = 0;      // 0 = bina 0-1, 1 = bina 2-3, 2 = bina 4
    private static final int PER_PAGE = 2;

    // Sayfadaki bina kartları
    private Table cardContainer;
    private Label pageLabel;
    private Texture frameTex;
    private Array<Texture> loadedTextures = new Array<>();

    public EducationPanel(BaseScreen rootScreen) {
        super(rootScreen, "Education");
        this.buildings = BuildingRegistry.getEducationBuildings();
    }
    ...
    public void buildLayout() {
        ...
        Label title = makeLabel("EDUCATION", 0x00E5FFFF, 2.0f, skin);
    ...
    private void refreshCards(Skin skin) {
        ...
        // Tek bina kaldıysa sol hizala
        if (end - start == 1) {
            cardContainer.add().expandX(); // sağ tarafı boş bırak
        }
    }
    ...
    private Table buildCard(Building building, Skin skin) {
        ...
        if (Gdx.files.internal(imgPath).exists()) {
            ...
            Label placeholder = makeLabel("[ No Image ]", 0x666666FF, 0.8f, skin);
            imgContainer.add(placeholder).size(220, 140).center();
        }
        card.add(imgContainer).fillX().padBottom(8).row();

        // Bina adı
        Label nameLbl = makeLabel(building.getName(), 0xFFFFFFFF, 1.5f, skin);
        card.add(nameLbl).center().padBottom(6).row();
        ...
        Label costLbl    = makeLabel("Construction:", 0xAABBCCFF, 1.0f, skin);
        Label costVal    = makeLabel("$" + formatNum(building.getConstructionCost()), 0xFFDD44FF, 1.1f, skin);
        Label budgetLbl  = makeLabel("Monthly:", 0xAABBCCFF, 1.0f, skin);
        Label budgetVal  = makeLabel("$" + formatNum(building.getMonthlyBudget()) + "/mo", 0xFF8844FF, 1.1f, skin);

        costTable.add(costLbl).left().padRight(6);
        costTable.add(costVal).left().row();
        costTable.add(budgetLbl).left().padRight(6);
        costTable.add(budgetVal).left().row();
        card.add(costTable).fillX().padBottom(8).row();

        // Etkilediği metrikler
        Label effectTitle = makeLabel("EFFECTS:", 0xAABBCCFF, 1.0f, skin);
        card.add(effectTitle).left().padBottom(4).row();
    ...
    private void onBuildClicked(Building building) {
        if (currentRegion == null) {
            Gdx.app.log("EducationPanel", "No region selected!");
            return;
        }
        // TO DO: bütçe kontrolü ve region metrik güncelleme
        Gdx.app.log("EducationPanel",
            "Building: " + building.getName() + " in " + currentRegion.getName());
    }
