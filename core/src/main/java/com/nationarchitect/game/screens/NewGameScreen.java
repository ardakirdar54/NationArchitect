...
import com.nationarchitect.game.ui.ChangeableButton;
...
public class NewGameScreen extends BaseScreen {
    ...
    public void buildLayout() {
        ...
        root.pad(60); // Padding artırıldı
        root.top().left();
        stage.addActor(root);

        // --- SOL SÜTUN (Harita ve Başlık) ---
        Table leftColumn = new Table();
        leftColumn.top().left();

        titleLabel = makeLabel("NEW GAME", 0x00E5FFFF, 3.5f); // Başlık büyütüldü
        leftColumn.add(titleLabel).left().padBottom(40).row();

        mapPreviewImage = new Image(mapPreviewTexture);
        leftColumn.add(mapPreviewImage).size(700, 350).left().padBottom(30).row(); // Harita ciddi oranda büyüdü

        mapSelector = new ChangeableButton(localSkin);
        mapSelector.addOption("map1");
        mapSelector.addOption("map2");
        leftColumn.add(mapSelector).width(450).height(80).left().row(); // Selector büyüdü

        // --- SAĞ SÜTUN (İsim ve Veriler) ---
        Table rightColumn = new Table();
        rightColumn.top().right();

        // İSİM YAZMA YERİ (Siyah yazı ayarı burada)
        TextField.TextFieldStyle tfStyle = new TextField.TextFieldStyle();
        tfStyle.font = localSkin.getFont("default-font");
        tfStyle.fontColor = Color.BLACK; // OYUNCU YAZISI SİYAH YAPILDI
        tfStyle.messageFontColor = new Color(0x333333FF); // Placeholder koyu gri
        ...
        rightColumn.add(nicknameField).width(600).height(80).padBottom(40).row(); // Input büyüdü

        Table nationalDataBox = buildNationalDataBox();
        rightColumn.add(nationalDataBox).width(600).padBottom(40).row(); // Veri kutusu genişletildi
        ...
        // Buton boyutları ciddi oranda artırıldı
    ...
    private void addResourceRow(Table parent, String name, float value) {
        Label lbl = makeLabel(name, 0xCCCCCCFF, 1.3f); // Yazılar büyüdü
        ProgressBar bar = new ProgressBar(0f, 1f, 0.01f, false, makeProgressBarStyle());
        bar.setValue(value);

        parent.add(lbl).left().width(250).padBottom(18);
        parent.add(bar).width(250).height(35).padBottom(18).row(); // Barlar büyüdü
    }
    ...
    // BorderDrawable metodu aynı kalabilir, sadece w ve h değerlerini içten genişletiyoruz
